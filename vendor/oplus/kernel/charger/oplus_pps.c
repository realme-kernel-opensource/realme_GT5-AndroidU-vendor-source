...
void oplus_pps_set_svooc_mos_enable(bool enable)
{
	...
	if (chip->pps_support_type == PPS_SUPPORT_3CP) {
		oplus_pps_charging_enable_slave(chip, enable);
		if(enable == TRUE)
			schedule_delayed_work(&chip->pps_svooc_en_cp_work, 0);
		else
			oplus_pps_charging_enable_slave_b(chip, enable);
	} else {
		oplus_pps_charging_enable_master(chip, enable);
...
static void oplus_pps_svooc_en_cp_work(struct work_struct *work)
{
	struct oplus_pps_chip *chip = container_of(work, struct oplus_pps_chip, pps_svooc_en_cp_work.work);
	int cnt_fail = 0;

	if (!chip || !chip->pps_support_type) {
		return;
	}
	mutex_init(&chip->pps_svooc_en_cp_lock);
	chg_err("en slave for svooc\n");
	mutex_lock(&chip->pps_svooc_en_cp_lock);
	msleep(100);
	if (chip->pps_support_type == PPS_SUPPORT_3CP) {
		for (cnt_fail = 0; cnt_fail < PPS_SVOOC_ENALBE_CHECK_CNTS; cnt_fail++) {
			chip->data.cp_slave_ibus = chip->ops->pps_get_cp_slave_ibus();
			if (chip->data.cp_slave_ibus < PPS_SVOOC_IBUS_ABNORMAL_MIN) {
				msleep(200);
				pps_err("ibus < 1a, %d ,cp_slave_ibus = %d\n", cnt_fail, chip->data.cp_slave_ibus);
			} else {
				pps_err("ibus > 1a, %d ,cp_slave_ibus = %d\n", cnt_fail, chip->data.cp_slave_ibus);
				break;
			}
		...
		oplus_pps_charging_enable_slave_b(chip, TRUE);
		mutex_unlock(&chip->pps_svooc_en_cp_lock);
	} else {
		for (cnt_fail = 0; cnt_fail < PPS_SVOOC_ENALBE_CHECK_CNTS; cnt_fail++) {
			chip->data.cp_master_ibus = chip->ops->pps_get_cp_master_ibus();
			if (chip->data.cp_master_ibus < PPS_SVOOC_IBUS_ABNORMAL_MIN) {
				msleep(200);
				pps_err("ibus < 1a, %d ,cp_master_ibus = %d\n", cnt_fail, chip->data.cp_master_ibus);
			} else {
				pps_err("ibus > 1a, %d ,cp_master_ibus = %d\n", cnt_fail, chip->data.cp_master_ibus);
				break;
			}
	...
	cancel_delayed_work(&chip->pps_svooc_en_cp_work);
}
...
int oplus_pps_init(struct oplus_chg_chip *g_chg_chip)
{
	...
	INIT_DELAYED_WORK(&chip->pps_svooc_en_cp_work, oplus_pps_svooc_en_cp_work);
